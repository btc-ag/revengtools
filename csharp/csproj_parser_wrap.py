...
from csharp.csproj_modules import CSharpModuleListSupply
from base.basic_config_if import BasicConfig
from csharp.csproj_parser import (CSProjParser, SetModuleResolver, CSProjChecker)
import logging
import os
...
class CSharpProjectReferenceDependencyParser(DependencyParser, ConfigDependent):

    def __init__(self, csproj_checker_class=CSProjChecker, module_list_supply=ModuleListSupply()):
        # TODO actually, this does not require an arbitrary ModuleListSupply, but a CSharpModuleListSupply, but
        ...
        self.__csproj_checker_class = csproj_checker_class
        if not hasattr(module_list_supply, "get_module_spec_files") or not hasattr(module_list_supply, "get_source_files") \
          or not hasattr(module_list_supply, "set_results") or not hasattr(module_list_supply, "set_checked_rules"):
            raise TypeError("CSharpProjectReferenceDependencyParser requires a %s" % (CSharpModuleListSupply,))
        self.__module_list_supply = module_list_supply
    
    def __create_module_checker_parameter(self, csproj):
        module_checker_parameter = dict()
        assembly_name = ""
        root_namespace = ""
        try:
            root_namespace = csproj.get_root_namespace()
        except:
            pass
        try:
            ...
            pass
        csproj_filename = csproj.get_filename()
        file_basename = os.path.basename(csproj_filename)
        file_dirname = self.__to_relative_name(os.path.dirname(csproj_filename))
        data = dict({ModuleCheckerParameterKeys.ANALYSIS_BASE_DIRS: self.__analysis_base_dirs,
                     ...
                     ModuleCheckerParameterKeys.SOURCE_FILES: list(self.__to_relative_name(res.name()) for res in csproj.get_source_files())})
    ...
    def __process_csproj(self, resource):
        parser = CSProjParser(resource.open())
    ...
    def process(self):
        ...
        checker = self.__csproj_checker_class(module_checker_parameter = self.__module_checker_parameter,
