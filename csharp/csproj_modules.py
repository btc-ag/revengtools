...
from csharp.csproj_parser import VSConstants, CSProjParser, CSProjParserTools
import logging
import os

class CSharpModuleListSupply(BaseModuleListSupply, ModuleListSupplyEx, ConfigDependent, SourceFileSupply, SolutionFileSupply, RuleSupply):
    ...
    def __initialize(self):
        ...
        for base_path in self.__project_dir_resolver.get_base_paths():
            for subset in self.__analysis_base_dirs:
                ...
                self.__logger.info("Parsing .csproj files in %s" % directory)
...
#        except ValueError:
#            self.__logger.warning(".csproj file %s does not declare an assembly name" % (parser.get_filename()), exc_info=1)
#        try:
#            names.append(parser.get_root_namespace())
#        except ValueError:
#            self.__logger.info(".csproj file %s does not declare a root namespace" % (parser.get_filename()), exc_info=1)
#        return sorted(names, key=lambda x: -len(x))[0]
    ...
    def __walk(self, _arg, dirname, filenames):
        for filename in filenames:
            if filename.endswith(VSConstants.CSPROJ_EXTENSION):
                fullname = os.path.join(dirname, filename) 
                self.__logger.info("Parsing .csproj file %s" % fullname)
                parser = CSProjParser(fullname)
                self.__module_files_to_names[fullname] = CSProjParserTools.get_best_name(parser, self.__project_dir_resolver)
                self.__modules_to_files[self.__module_files_to_names[fullname]] = tuple(parser.get_source_files())
                #checker = config_csproj_checker_class(parser)
            ...
            """
            if filename.endswith(VSConstants.SOURCE_EXTRENSION):
